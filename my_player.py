...
class MyPlayer(PlayerDivercite):
    def __init__(self, piece_type: str, name: str = "MyPlayer"):
        ...
        # --------- Poids pour l'évaluation de l'état ---------
        self.W_score = 0.5
    ...
    def _get_state_hash(self, state: GameStateDivercite) -> tuple:
        """
        Genere un tuple avec les pieces du terrain et l'id du prochain joueur

        Args:
            state (GameStateDivercite): L'état du jeu à hacher

        Returns:
            tuple: Un tuple représentant l'état du jeu
        """
        board_items = state.get_rep().get_env().items()
        board_tuple = tuple(sorted(
            (pos, piece.get_type(), piece.get_owner_id())
            for pos, piece in board_items
        ))
        player_id = state.get_next_player()
        return (board_tuple, player_id)

    def compute_action(self, current_state: GameStateDivercite, remaining_time: int = 1e9) -> Action:
        """
        Calcule l'action à effectuer en fonction de l'état actuel du jeu.

        Args:
            current_state (GameStateDivercite): L'état actuel du jeu.
            remaining_time (int, optional): Le temps restant pour jouer la partie
        ...
        elif game_progress >= 15:
            time_for_this_move = remaining_time * 0.15
        else:
            time_for_this_move = (remaining_time / moves_left) * 0.98
            time_for_this_move = max(0.5, min(time_for_this_move, 120))

        best_action = None
        current_depth = 1

        # On peut faire un tri initial sur les actions possibles
    ...
    def minimax(self, state, depth, time_limit, alpha=float('-inf'), beta=float('inf'), maximizing_player=True):
        """
        Implémente l'algorithme Minimax avec élagage alpha-bêta.

        Args:
            state (GameStateDivercite): L'état actuel du jeu.
            depth (int): La profondeur de recherche actuelle.
            time_limit (int): Le temps limite pour la recherche.
            alpha (float): La valeur alpha pour l'élagage.
            beta (float): La valeur bêta pour l'élagage.
            maximizing_player (bool): Indique si le joueur actuel est le maximiseur.

        Returns:
            tuple: L'action optimale et la valeur associée.
        """
        elapsed_time = time.time() - self.start_time
        ...
        if maximizing_player:
            ...
            for action in ordered_actions:
                ...
                try:
                    _, eval = self.minimax(
                        next_state, depth - 1, time_limit, alpha, beta, False)
                except TimeoutError:
                    raise
            ...
            for action in ordered_actions:
                ...
                try:
                    _, eval = self.minimax(
                        next_state, depth - 1, time_limit, alpha, beta, True)
                except TimeoutError:
                    raise
            ...
            self.transposition_table[state_hash] = {
                'depth': depth, 'value': min_eval}
            return best_action, min_eval

    def order_moves(self, actions, state, maximizing_player) -> list:
        """
        Trie les actions possibles en fonction de leur score grace à notre heuristique.

        Args:
            actions (list): Liste des actions possibles.
            state (GameStateDivercite): L'état du jeu actuel.
            maximizing_player (bool): Indique si le joueur actuel est le maximiseur.

        Returns:
            list: Liste des actions triées.
        """
        move_scores = []
        my_id = self.get_id()
    ...
    def evaluate_state(self, state: GameStateDivercite) -> float:
        """
        Évalue l'état du jeu en fonction de la position des pièces et des scores.

        Args:
            state (GameStateDivercite): L'état à évaluer.

        Returns:
            float: La valeur heuristique de l'état.
        """
    ...
    def calculate_opening_bias(self, state: GameStateDivercite) -> float:
        ...
        Args:
            state (GameStateDivercite): L'état du jeu à évaluer.
        Returns:
            float: La valeur du biais d'ouverture.
        """
        bias = 0
        center_pos = (4, 4)
    ...
    def calculate_state_heuristic(self, state: GameStateDivercite) -> float:
        """
        Calcule la valeur heuristique de pour un état donné.
    ...
    def evaluate_city_position(self, state: GameStateDivercite, pos: tuple, city_color: str) -> float:
        """
        Évalue la position d'une ville donné

        Args:
            state (GameStateDivercite): L'état du jeu auquel la ville est évaluée.
            pos (tuple): La position de la ville.
            city_color (str): La couleur de la ville.
        Returns:
            float: La valeur de la position de la ville.
        """
    ...
    def evaluate_resource_position(self, state: GameStateDivercite, pos: tuple, ressource_color: str) -> float:
        """
        Évalue la position d'une ressource donnée.
        ...
        On evalue l'impact de la ressource sur la cite:
            ...
            - Si la ressource est de couleur différente, on regarde si il y a
              au moins 2 ressources de cette couleur, si oui on applique un
              malus sur le gain de la ressource

        Args:
            state (GameStateDivercite): L'état du jeu auquel la ressource est évaluée.
            pos (tuple): La position de la ressource.
            ressource_color (str): La couleur de la ressource.
        Returns:
            float: La valeur de la position de la ressource.
        """
        neighbors = state.get_neighbours(pos[0], pos[1])
        impact = 0
        my_id = self.get_id()
